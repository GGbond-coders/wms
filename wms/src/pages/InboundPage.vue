...
import { apiInboundCreate, apiInboundList, apiUsers } from '../api/wms'
...
async function load() {
  ...
  try {
    ...
    const data = await apiInboundList({ goodsName: query.goodsName.trim(), page: page.value, pageSize: pageSize.value })
...
async function save() {
  ...
  try {
    ...
    await apiInboundCreate({ goodsId, quantity, operatorId })
...
<template>
  <div class="panel">
      <div class="panelHeader">
        <div class="panelTitle">入库管理</div>
        <div class="row">
          <button class="ghostBtn" :disabled="loading" @click="load">刷新</button>
          <button class="primaryBtn" :disabled="!isAdmin" @click="openCreate">新增入库</button>
        </div>
      </div>
    <div class="panelBody">
  ...
  <Modal :open="createOpen" title="新增入库" @close="createOpen = false">
    <div class="grid">
      <div class="field">
